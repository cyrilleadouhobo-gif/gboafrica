...
export default function LegalPage({ params }) {
  ...
  return (
    <div style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(80px,10vw,120px) clamp(20px,5vw,40px) clamp(64px,9vw,110px)' }}>
      ...
      <h1 style={css("font-family:'Space Grotesk';font-weight:700;font-size:clamp(30px,5vw,46px);letter-spacing:-1px;margin-bottom:6px")}>{legal.title}</h1>
      <div style={css('font-size:13px;color:var(--muted,#8a8a8a);margin-bottom:34px')}>{legal.updated}</div>
      <div style={{ display: 'grid', gap: 26 }}>
        {legal.sections.map((sec) => (
          <div key={sec.h}>
            <h2 style={css("font-family:'Space Grotesk';font-weight:700;font-size:20px;margin-bottom:10px")}>{sec.h}</h2>
