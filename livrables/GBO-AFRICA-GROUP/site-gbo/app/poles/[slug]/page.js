...
export default function PolePage({ params }) {
  ...
  const mono = (POLES.find((p) => p.key === slug) || {}).mono || 'G';
    ...
    <div>
      <section style={css('position:relative;padding:clamp(80px,10vw,130px) clamp(20px,5vw,64px) clamp(48px,6vw,80px);overflow:hidden;background:#060606')}>
        ...
        <div style={{ position: 'relative', maxWidth: 1100, margin: '0 auto' }}>
          ...
          <h1 style={css("font-family:'Broaven';font-weight:700;font-size:clamp(30px,7vw,66px);letter-spacing:-2px;line-height:.98;color:#fff")}>{detail.name}</h1>
        ...
        <div style={css('max-width:640px;margin:0 auto;padding:clamp(28px,4vw,44px);border-radius:24px;background:var(--lime,#C6F202);color:#000;text-align:center')}>
          {done ? (
            <div style={{ animation: 'fadeUp .4s both' }}>
              ...
              <h3 style={css("font-family:'Broaven';font-weight:700;font-size:24px;margin-bottom:8px")}>Vous êtes sur la liste !</h3>
            ...
            <form onSubmit={submitWaitlist}>
              <h3 style={css("font-family:'Broaven';font-weight:700;font-size:clamp(22px,3.5vw,30px);margin-bottom:8px")}>Soyez prévenu au lancement</h3>
