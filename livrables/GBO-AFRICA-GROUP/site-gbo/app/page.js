...
export default function HomePage() {
  ...
  return (
    <div>
        ...
        <div style={css('position:relative;max-width:1200px;margin:0 auto;width:100%;padding:0 clamp(20px,5vw,64px) clamp(56px,9vw,110px);pointer-events:none')}>
          <div style={{ pointerEvents: 'auto' }}>
            ...
            <div style={css('display:flex;flex-wrap:wrap;gap:14px;margin-top:34px;animation:fadeUp .7s .2s both')}>
              <Link href="/fitness" style={css('padding:16px 30px;border-radius:12px;background:#C6F202;color:#000;font-weight:700;font-size:16px')}>
                Commencer
              </Link>
              <button
          ...
          <div style={css('display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px')}>
            {VALUES.map((v) => (
              <div
                key={v.n}
                style={css(
                  'padding:24px;border-radius:18px;border:1px solid var(--border,rgba(255,255,255,.09));background:var(--glass,rgba(255,255,255,.02));transition:.2s'
                )}
              >
                <div
                  style={css(
          ...
          <div style={css('display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px')}>
            {poles.map((p) => (
              <Link
                ...
                style={css('display:block;padding:26px;border-radius:20px;border:1px solid var(--border,rgba(255,255,255,.1));background:var(--glass,rgba(255,255,255,.02));cursor:pointer;transition:.22s')}
              >
      ...
      <section style={css('padding:clamp(40px,6vw,72px) clamp(20px,5vw,64px);background:var(--surface,#0b0b0b);overflow:hidden')}>
      ...
      <section style={css('padding:clamp(64px,10vw,130px) clamp(20px,5vw,64px);background:var(--lime,#C6F202);color:#000')}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          ...
          <Link href="/fitness" style={css('padding:18px 40px;border-radius:14px;background:#000;color:#C6F202;font-weight:700;font-size:17px;display:inline-block')}>
