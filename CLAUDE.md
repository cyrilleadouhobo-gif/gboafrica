...
> **Note sur l'emplacement :** ce dossier est imbriqué sous `Downloads/jarvis-starter-kit/jarvis-starter-kit/` (deux niveaux). C'est ici, dans le dossier interne, que se trouve le vrai contenu du kit (ce fichier, `context/`, `.claude/`, `module-installs/`). Si une session Claude Code démarre au niveau du dossier parent, elle doit lire ce fichier en entier avant toute chose.

---

## What This Is

Ce workspace est le Jarvis personnel de Cyrille. Il a été créé avec le Jarvis Starter Kit pour servir d'assistant IA personnel au quotidien.
...
Je m'appelle Cyrille et je vis à Abidjan, avec des allers-retours réguliers à Bouaké et N'douci. Je suis informaticien de formation (licence), actuellement employé comme pompiste chez TotalEnergies Côte d'Ivoire, et en parallèle actionnaire et COO de GBÔ AFRICA GROUP, une SARL que je co-fondée avec mon meilleur ami, axée sport et bien-être.

Mes objectifs prioritaires actuels sont de réussir un concours de la fonction publique (ou à défaut décrocher une promotion au poste de caissier), et de lancer concrètement les activités de GBÔ pour générer du revenu.

À long terme, je veux construire une activité qui me génère un revenu stable, développer une compétence reconnue en informatique, et atteindre une vraie stabilité financière.

Le domaine où j'ai besoin du plus d'aide en ce moment : mener à bien ma responsabilité de COO au sein de GBÔ.
